export default async function handler(req, res) {
  ...
  async function buildEvaluation(p,milestone) {
    ...
    const prompt=`Evaluate the progress of an 18-year-old French learner after ${milestone} completed conversation sessions. This is a coaching progress report, not a formal exam. Be honest, constructive and specific. Do not infer a precise CEFR level unless the evidence is strong. Use the recorded conversation excerpts and activity metrics only; do not invent facts. Keep each assessment to 1-2 sentences, each list item to one sentence, and nextGoal to one sentence. Write in English because the parent report is in English. Be encouraging but do not flatter without evidence. Metrics: ${p.sessionsCompleted} sessions completed; ${fullDays} full days (two articles + conversation); ${vocabDays} vocabulary-bilan days; ${rated.length} articles rated; average interest ${avg('interest')===null?'n/a':avg('interest').toFixed(1)}/5; average difficulty ${avg('difficulty')===null?'n/a':avg('difficulty').toFixed(1)}/10. Recent articles:\n${articleList||'None'}\nConversation excerpts:\n${transcript||'No transcript available.'}`;
    const schema={type:'object',additionalProperties:false,properties:{french:{type:'object',additionalProperties:false,properties:{level:{type:'string'},assessment:{type:'string'}},required:['level','assessment']},criticalThinking:{type:'object',additionalProperties:false,properties:{assessment:{type:'string'}},required:['assessment']},knowledge:{type:'object',additionalProperties:false,properties:{assessment:{type:'string'}},required:['assessment']},strengths:{type:'array',items:{type:'string'}},focus:{type:'array',items:{type:'string'}},nextGoal:{type:'string'}},required:['french','criticalThinking','knowledge','strengths','focus','nextGoal']};
    const apiKey=process.env.OPENAI_API_KEY; if(!apiKey) throw new Error('OPENAI_API_KEY is not configured.');
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5-mini',store:false,input:prompt,text:{format:{type:'json_schema',name:'progress_evaluation',strict:true,schema}},max_output_tokens:900})});
    ...
    return {parsed:JSON.parse(text),fullDays,vocabDays,ratedCount:rated.length,avgInterest:avg('interest'),avgDifficulty:avg('difficulty')};
  }
  try {
