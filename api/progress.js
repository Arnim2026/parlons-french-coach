export default async function handler(req, res) {
  ...
  async function buildEvaluation(p,milestone) {
    ...
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5-mini',store:false,input:prompt,text:{format:{type:'json_object'}},max_output_tokens:1800})});
    ...
    if(data?.status==='incomplete') throw new Error(`Evaluation output incomplete: ${clampText(data?.incomplete_details?.reason||'unknown reason',120)}`);
    let parsed;
